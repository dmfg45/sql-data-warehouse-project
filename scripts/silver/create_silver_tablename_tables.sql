...
================================================================================
Purpose:
    This script dynamically recreates a set of "silver" tables in the Data Warehouse.
    It performs the following steps for each table defined in the @Tables table variable:

    1. Checks if the table already exists in the target schema ('silver').
       - If it exists, the table is dropped.
    2. Creates the table from scratch using the corresponding column definitions.
    3. Iterates through all tables defined in @Tables to ensure they are all created fresh.
    
    Tables handled in this script include:
        - crm_cust_info
        - crm_prd_info
        - crm_sales_details
        - erp_cust_az12
...
Usage:
    Simply run this script in the target database to recreate the silver tables.
    All existing data in the tables will be lost since the tables are dropped before creation.

Notes:
    - Uses a cursor to iterate through table definitions stored in a table variable.
    - Dynamic SQL (sp_executesql) is used to drop and create tables.
================================================================================
*/
...
DECLARE @SchemaName NVARCHAR(128) = 'silver';
